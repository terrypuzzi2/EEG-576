...
channelsToCorrelate = [1, 2, 3, 4, 5, 6];
...
if exist(icaFileName, 'file')
    ...
    for channel = 1:6
        for i = 1:length(fileNames)
            ...
            filteredSignal = filtfilt(b, a, eegSignals);
            
            %% Perform ICA
            [weights, sphere, ~, ~, ~, ~] = runica(filteredSignal', 'pca', 6); % Assuming 6 components
            icaSignals = weights * filteredSignal'; % ICA decomposition
...
for channel = 1:6
    for i = 1:length(fileNames)
        ...
        for j = 1:length(timeIntervals)
            ...
            n = size(dataSegment, 1); % Number of samples in the segment
            ...
            fftResult = fft(dataSegment(:, channel));
...
% Apply bandpass filter before ICA
[b, a] = butter(4, [lowCutoff, highCutoff] / (fs / 2), 'bandpass');
filteredNewSignals = filtfilt(b, a, newEEGSignals);

% Perform ICA decomposition on the filtered data
[weights, sphere, ~, ~, ~, ~] = runica(filteredNewSignals', 'pca', 6); % Assuming 6 components
icaSignalsNew = weights * filteredNewSignals'; % ICA decomposition
